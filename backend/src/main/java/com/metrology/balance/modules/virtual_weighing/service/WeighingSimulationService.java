package com.metrology.balance.modules.virtual_weighing.service;
...
@Slf4j
@Service
public class WeighingSimulationService {
    ...
    public VirtualWeighingResult performWeighing(List<Integer> leftItemIds, List<Integer> rightItemIds,
        ...
        double leftTorqueNm = leftMass / 1000.0 * leftArm / 1000.0 * BalancePhysicsEngine.GRAVITY;
        double rightTorqueNm = rightMass / 1000.0 * rightArm / 1000.0 * BalancePhysicsEngine.GRAVITY;
        double torqueDifferenceNm = leftTorqueNm - rightTorqueNm;
        double torqueDifferenceGmm = leftMass * leftArm - rightMass * rightArm;
        result.setTorqueDifference(torqueDifferenceGmm);

        PhysicsParameters physicsParams = BalancePhysicsEngine.buildPhysicsParameters(
                leftArm, rightArm, leftMass, rightMass, beamMass, knifeEdgeRadius, beamMaterial);

        double swingAngle = BalancePhysicsEngine.calculateSwingAngle(
                torqueDifferenceNm, physicsParams.getTorsionalStiffnessNmPerRad(), physicsParams.getDampingRatioXi());
        result.setSwingAngle(swingAngle);

        double equilibriumTime = BalancePhysicsEngine.calculateEquilibriumTime(
                physicsParams.getDampingRatioXi(), physicsParams.getNaturalFrequencyRadPerS());
        result.setEquilibriumTime(equilibriumTime);

        String balanceStatus;
        double tolerance = 0.5;
        if (Math.abs(torqueDifferenceGmm) < tolerance) {
            balanceStatus = "BALANCED";
        } else if (torqueDifferenceGmm > 0) {
        ...
        double relativeError = Math.abs(torqueDifferenceGmm) / Math.max(maxTorque, 0.01);
        ...
        Map<String, Object> physicsParamsMap = buildPhysicsParamsMap(physicsParams);
        result.setPhysicsParameters(physicsParamsMap);

        Map<String, Object> animationData = buildAnimationData(
                swingAngle, physicsParams.getDampingRatioXi(),
                physicsParams.getNaturalFrequencyRadPerS(), physicsParams.getDampedFrequencyRadPerS(),
                equilibriumTime);
        result.setAnimationData(animationData);

        List<String> insights = generateInsights(leftItems, rightItems, leftMass, rightMass,
                torqueDifferenceGmm, balanceStatus);
        ...
        log.info("虚拟称量完成: 左盘{}件({}g), 右盘{}件({}g), 状态={}",
    ...
    private Map<String, Object> buildPhysicsParamsMap(PhysicsParameters params) {
        Map<String, Object> physicsParams = new LinkedHashMap<>();
        physicsParams.put("leftArmLength_mm", params.getLeftArmLengthMm());
        physicsParams.put("rightArmLength_mm", params.getRightArmLengthMm());
        physicsParams.put("leftTorque_gmm", params.getLeftTorqueGmm());
        physicsParams.put("rightTorque_gmm", params.getRightTorqueGmm());
        physicsParams.put("beamMomentOfInertia_kgm2", params.getBeamMomentOfInertiaKgm2());
        physicsParams.put("totalMomentOfInertia_kgm2", params.getTotalMomentOfInertiaKgm2());
        physicsParams.put("torsionalStiffness_Nm_per_rad", params.getTorsionalStiffnessNmPerRad());
        physicsParams.put("materialDampingCoefficient", params.getMaterialDampingCoefficient());
        physicsParams.put("airDampingCoefficient", params.getAirDampingCoefficient());
        physicsParams.put("frictionDampingCoefficient", params.getFrictionDampingCoefficient());
        physicsParams.put("totalDampingCoefficient", params.getTotalDampingCoefficient());
        physicsParams.put("naturalFrequency_rad_per_s", params.getNaturalFrequencyRadPerS());
        physicsParams.put("dampingRatio_xi", params.getDampingRatioXi());
        physicsParams.put("dampedFrequency_rad_per_s", params.getDampedFrequencyRadPerS());
        physicsParams.put("gravity_m_per_s2", params.getGravityMPerS2());
        physicsParams.put("oscillationFrequency_Hz", params.getOscillationFrequencyHz());
        physicsParams.put("beamMass_g", params.getBeamMassG());
        physicsParams.put("knifeEdgeRadius_mm", params.getKnifeEdgeRadiusMm());
        return physicsParams;
    }

    private Map<String, Object> buildAnimationData(double swingAngle, double dampingRatio,
                                                    double naturalFrequency, double dampedFrequency,
                                                    double equilibriumTime) {
        Map<String, Object> animationData = new LinkedHashMap<>();
        List<OscillationFrame> oscillationFrames = BalancePhysicsEngine.generateOscillationFrames(
                swingAngle, dampingRatio, naturalFrequency, dampedFrequency, equilibriumTime);

        List<Map<String, Object>> frameMaps = oscillationFrames.stream()
                .map(frame -> {
                    Map<String, Object> frameMap = new LinkedHashMap<String, Object>();
                    frameMap.put("time_s", frame.getTimeS());
                    frameMap.put("angle_deg", frame.getAngleDeg());
                    frameMap.put("angularVelocity_deg_per_s", frame.getAngularVelocityDegPerS());
                    frameMap.put("envelope", frame.getEnvelope());
                    return frameMap;
                })
                .collect(Collectors.toList());
        ...
        animationData.put("oscillationFrames", frameMaps);
